...
var SchedulesURL = ""
var SplatNetGearURL = ""
var LangjsonURL = "https://splatoon3.ink/data/locale/ja-JP.json"
...
const interactiondata = [
    ...
    {name: "Xmatch",description: "Xマッチのステージを取得します。",options: [{type: "INTEGER",name: "int",description: "何個先のスケジュールまで表示するか書いてください。(デフォルト2最大5)",required: false}]   },
    ...
    {name: "coop",description: "サーモンランの情報を取得します。",options: [{type: "INTEGER",name: "int",description: "何個先のスケジュールまで表示するか書いてください。(デフォルト2最大4)",required: false}]   },
];
...
sclient.on("interactionCreate", async (interaction) => {
  ...
  if(interaction.commandName === 'series'){
    ...
    //int確認
    if(interaction.options.getInteger('int') < 1||interaction.options.getInteger('int') > 5){
      if(interaction.options.getInteger('int') < 1)limitc = 1;
      if(interaction.options.getInteger('int') > 5)limitc = 5;
    }else{
  ...
  if(interaction.commandName === 'open'){
    ...
    //int確認
    if(interaction.options.getInteger('int') < 1||interaction.options.getInteger('int') > 5){
      if(interaction.options.getInteger('int') < 1)limito = 1;
      if(interaction.options.getInteger('int') > 5)limito = 5;
    }else{
  ...
  if(interaction.commandName === 'Xmatch'){
    ...
    //int確認
    if(interaction.options.getInteger('int') < 1||interaction.options.getInteger('int') > 5){
      if(interaction.options.getInteger('int') < 1)limitX = 1;
      if(interaction.options.getInteger('int') > 5)limitX = 5;
    }else{
      limitX = interaction.options.getInteger('int')
    }
    
    for (let i = 0; i < limitX ; i++) {
      if(Xsc.nodes[i].xMatchSetting !== null){
          var rule = langdata.rules[Xsc.nodes[i].xMatchSetting[1].vsRule.id].name
          var Stage1JP = langdata.stages[Xsc.nodes[i].xMatchSetting[1].vsStages[0].id].name
          var Stage2JP = langdata.stages[Xsc.nodes[i].xMatchSetting[1].vsStages[1].id].name
          ...
          .setImage(Xsc.nodes[i].xMatchSetting[1].vsStages[0].image.url)
          )
          Xembed.push(new Discord.MessageEmbed()
          .setURL('https://discord.com/channels/588336492049465364/588343228063940619/1038374680240861234%index='+String(i))
          .setImage(Xsc.nodes[i].xMatchSetting[1].vsStages[1].image.url))
          }
  ...
  if(interaction.commandName === 'coop'){
    ...
    //int確認
    if(interaction.options.getInteger('int') < 1||interaction.options.getInteger('int') > 5){
      if(interaction.options.getInteger('int') < 1)limitco = 1;
      if(interaction.options.getInteger('int') > 4)limitco = 4;
    }else{
    ...
    if(i + 1 < limitco && coopsc.nodes[i+1].startTime !== coopsc.nodes[i].endTime){
